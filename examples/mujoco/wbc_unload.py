"""Go2 four-foot support with a smooth backward-left base shift."""
...
def run(args):
    ...
    fr_index = controller.contact_frame_names.index("FR_foot")
    ...
    print(
        " time  FR_limit   FR_QP   FR_MJ"
        "   xy_err_mm       z  feet  max_tau"
    )
        ...
        try:
            while data.time < args.duration:
                ...
                normal_force_limits = np.full(
                    len(controller.contact_frame_names),
                    controller.maximum_normal_force,
                )

                normal_force_limits[fr_index] = (
                    controller.maximum_normal_force * (1.0 - unload_progress)
                )
                ...
                if result.solver_status.lower() != "solved":
                    ...
                    raise RuntimeError(
                        ...
                        f"time={t:.4f} s, "
                        f"FR_limit={normal_force_limits[fr_index]:.6f} N"
                    )
                ...
                if t >= next_print_time:
                    xy_error_mm = 1000.0 * np.linalg.norm(
                    ...
                    print(
                        f"{t:5.2f}"
                        f" {normal_force_limits[fr_index]:9.3f}"
                        f" {fr_qp_force:7.3f}"
...
def main():
    ...
    if not np.isfinite(values).all() or args.duration <= 4.0:
        parser.error(
            "Expected finite arguments and duration > 4 s"
        )
