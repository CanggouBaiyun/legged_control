"""Go2 single-leg lift, touchdown and four-contact recovery."""
...
def run(args):
    ...
    lower_start_time = 13.0
    lower_duration = 2.0
    reload_duration = 2.0

    #连续检测到一定接触力，才确认触地
    touchdown_force_threshold = 1.0 # N
    touchdown_confirm_duration = 0.02 # s
    touchdown_timer = 0.0
    ...
    with viewer_context as viewer:
        ...
        try:
            while data.time < args.duration:
                ...
                if not lifting:
                ...
                elif touchdown_time is not None:
                    # 已确认触地：恢复四足约束
                    controller = four_contact_controller

                    reload_s = np.clip(
                        (t - touchdown_time) / reload_duration,
                        0.0,
                        1.0,
                    )
                    reload_progress = (
                        10.0 * reload_s**3 - 15.0 * reload_s**4 + 6.0 * reload_s**5
                    )

                    phase = (
                        "reload" if reload_s < 1.0 else "stand4"
                    )
                    ...
                    normal_force_limits[fr_index] = (
                        controller.maximum_normal_force * reload_progress
                    )

                    # 落地后保持触地点附近的参考，不再继续向下运动
                    foot_positions_ref[fr_index] = touchdown_position
                    ...
                    if t < lift_start_time + lift_duration:
                        phase = "lift"
                    elif t < lower_start_time:
                        phase = "hold"
                    elif t < lower_start_time + lower_duration:
                        phase = "lower"
                    else:
                        phase = "wait"
                    ...
                    if t < lower_start_time:
                        ...
                        # 用同一条五次曲线，生成向下运动。
                        rise_position, rise_velocity, rise_acceleration = (
                            vertical_lift_reference(
                                time=t - lower_start_time,
                                initial_position=feet[fr_index],
                                lift_height=lift_height,
                        ...
                        top_position = (
                            feet[fr_index] + np.array([0.0, 0.0, lift_height])
                        )

                        swing_position = (
                            top_position - (rise_position - feet[fr_index])
                        )
                        swing_velocity = -rise_velocity
                        swing_acceleration = -rise_acceleration

                        # 接触力来自最近一次仿真动力学计算。
                        fr_touch_force = read_foot_normal_force(
                            model, data, fr_geom_id, floor_id,
                        )

                        near_ground = (
                            abs(fr_position[2] - feet[fr_index, 2])
                            < 0.006
                        )

                        if (
                            fr_touch_force > touchdown_force_threshold
                            and near_ground
                        ):
                            touchdown_timer += model.opt.timestep
                        else:
                            touchdown_timer = 0.0

                        if touchdown_timer >= touchdown_confirm_duration:
                            touchdown_time = t
                            touchdown_position = fr_position.copy()

                            # 确认触地后，停止继续向下给参考。
                            swing_position = touchdown_position.copy()
                            swing_velocity = np.zeros(3)
                            swing_acceleration = np.zeros(3)
                            phase = "touch"

                        elif t > lower_start_time + lower_duration + 2.0:
                            raise RuntimeError(
                                "Touchdown timeout: "
                                f"FR force={fr_touch_force:.3f} N, "
                                f"FR z={fr_position[2]:.4f} m"
                            )
    ...
    summary = {
        ...
        "four_contact_recovery_completed": bool(
            touchdown_time is not None
            and data.time >= touchdown_time + reload_duration
        ),
    }
