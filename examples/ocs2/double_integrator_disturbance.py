...
def main():
    ...
    disturbance_velocity = 1.0
    disturbance_step = int(round(disturbance_time / dt))
    ...
    with TemporaryDirectory(prefix="ocs2_double_integrator_") as library_folder:
        ...
        for step in range(number_of_steps):
            ...
            if step == disturbance_step:
                velocity_before = state[1]
                state[1] += disturbance_velocity

                print(
                    f"\nVelocity disturbance at {current_time:.2f} s: "
                    f"{velocity_before:.5f} -> {state[1]:.5f} m/s\n"
                )
