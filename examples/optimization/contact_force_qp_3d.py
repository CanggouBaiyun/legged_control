...
dynamics_constraint = sparse.csc_matrix(base_force_mapping)

force_bound_constraint = sparse.eye(
    number_of_contact_force_variables, format="csc",
)
...
minimum_force_per_foot = np.array([
    -100.0,
    -100.0,
    0.0,
])

maximum_force_per_foot = np.array([
    100.0,
    100.0,
    100.0,
])

minimum_contact_forces = np.tile(
    minimum_force_per_foot,
    number_of_feet,
)

maximum_contact_forces = np.tile(
    maximum_force_per_foot,
    number_of_feet,
)

lower_bound = np.concatenate([
    required_generalized_force[:6],
    minimum_contact_forces,
])

upper_bound = np.concatenate([
    required_generalized_force[:6],
    maximum_contact_forces,
])
