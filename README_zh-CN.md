...
名义姿态下模型质量为 `16.087 kg`，四足竖直力总和为 `157.81347 N`，完整静态动力学残差范数约为 `9.12e-14`。

正在实现：

- 非零期望加速度下的浮动基动力学；
- 显式执行器选择矩阵 `S`；
- 从无约束最小二乘过渡到带约束的接触力 QP。

## 开发路线

- M4：使用两个求解器实现接触力分配 QP；
...
浮动基前 6 维没有直接驱动，所需的基座合力和力矩必须由足端接触产生。当前静态算例分步计算接触力和关节力矩；后续带约束 WBC 会同时优化 `dv`、`lambda` 和 `tau`。
